...
(new CustomEChart({
  ...
  options: () => ({
    ...
    xAxis: {
      ...
      axisLine: { lineStyle: { color: theme("body-secondary-color") } },
    },
    yAxis: {
      type: "value",
    ...
    tooltip: {
      ...
      formatter: function (e) {
        var t = e[0].name;
        let o = `<div style="font-size: 14px; font-weight: 600; text-transform: uppercase; border-bottom: 1px solid ${theme("border-color")}; margin-bottom: 8px; padding: 3px 10px 8px;">${t}</div>`;
        return (
          e.forEach((e) => {
            o += `<div style="margin-top: 4px; padding: 3px 15px;">
                            <span style="display:inline-block;margin-right:5px;border-radius:50%;width:10px;height:10px;background-color:${e.color};"></span>
                            ${e.seriesName} : <strong>${e.value}</strong>
                        </div>`;
          }),
          o
        );
    ...
    series: [
      {
        ...
        type: "line",
        areaStyle: { opacity: 0.2, color: theme("chart-primary") },
  ...
  new CustomEChart({
    ...
    options: () => ({
      ...
      xAxis: {
        ...
        axisTick: { show: !1 },
        axisLabel: { color: theme("body-color"), margin: 15 },
        splitLine: { show: !1 },
      },
      yAxis: {
        type: "value",
        splitLine: {
          lineStyle: { color: theme("border-color"), type: "dashed" },
        },
        boundaryGap: !1,
        axisLabel: { show: !0, color: theme("body-color"), margin: 15 },
        axisTick: { show: !1 },
        axisLine: { show: !1 },
      },
      series: [
        {
          ...
          areaStyle: { opacity: 0.2, color: theme("chart-delta") },
          lineStyle: { color: theme("chart-delta") },
          symbol: "circle",
          stack: "product",
        ...
        },
        {
          ...
          areaStyle: { opacity: 0.2, color: theme("chart-alpha") },
          lineStyle: { color: theme("chart-alpha") },
          symbol: "circle",
          stack: "product",
        ...
        },
        {
          ...
          areaStyle: { opacity: 0.2, color: theme("chart-beta") },
          lineStyle: { color: theme("chart-gamma") },
          symbol: "circle",
          stack: "product",
      ...
      grid: { right: 22, left: 5, bottom: 5, top: 8, containLabel: !0 },
    }),
  }),
  new CustomEChart({
    ...
    options: () => ({
      ...
      xAxis: {
        ...
        boundaryGap: !1,
        axisLine: { lineStyle: { color: theme("tertiary-bg"), type: "solid" } },
        axisTick: { show: !1 },
        axisLabel: { color: theme("body-color"), margin: 15 },
        splitLine: { show: !1 },
      },
...
let data = [],
  now = new Date(2023, 9, 3),
  oneDay = 864e5,
  value = 100 * Math.random();
for (let e = 0; e < 1e3; e++) data.push(randomData());
let dynamicChart = new CustomEChart({
  ...
  options: () => ({
    ...
    tooltip: {
      ...
      formatter: function (e) {
        ...
        return (
          t.getDate() +
          ` ${t.toLocaleString("default", { month: "long" })}, ${t.getFullYear()} : ` +
          e.value[1]
        );
      },
      axisPointer: { animation: !1, type: "none" },
      padding: [12, 16],
      backgroundColor: theme("secondary-bg"),
      borderColor: theme("border-color"),
      textStyle: { color: theme("light-text-emphasis") },
...
(setInterval(function () {
  for (let e = 0; e < 5; e++) (data.shift(), data.push(randomData()));
  dynamicChart.chart.setOption({ series: [{ data: data }] });
}, 1e3),
  new CustomEChart({
