...
(new CustomApexChart({
  ...
  new CustomApexChart({
    ...
    options: () => ({
      ...
      dataLabels: {
        ...
        offsetY: -25,
        style: { fontSize: "12px", colors: [theme("body-color")] },
      },
  ...
  new CustomApexChart({
    ...
    options: () => ({
      ...
      fill: { opacity: 1 },
      colors: [
        theme("primary-rgb", 0.7),
        theme("chart-primary"),
        theme("secondary-rgb", 0.7),
        theme("chart-secondary"),
      ],
  ...
  new CustomApexChart({
    ...
    options: () => ({
      ...
      xaxis: {
        tickPlacement: "on",
        axisBorder: { show: !0, color: theme("chart-order-color") },
        axisTicks: { show: !0, color: theme("chart-order-color") },
      },
      yaxis: { labels: { offsetX: -10 } },
    }),
  }),
        ...
        points: [
          {
            ...
            label: {
              ...
              offsetY: 0,
              style: { color: "#fff", background: theme("chart-secondary") },
    ...
    options: () => ({
      ...
      plotOptions: {
        bar: {
          colors: {
            ranges: [
              { from: -100, to: -46, color: theme("chart-alpha") },
              { from: -45, to: 0, color: theme("chart-beta") },
            ],
          },
          columnWidth: "80%",
          borderRadius: 6,
...
function makeData() {
  ...
  let a = ["primary", "secondary", "info", "danger", "warning", "success"];
  return e.map((e, t) => ({
    x: String(2019 + t),
    y: e.y,
    colorToken: a[t],
...
function updateQuarterChart(e, t) {
  var a = e.w.globals.selectedDataPoints[0];
  let r = e.w.config.series[0],
    o = [],
    s = [];
  (a?.forEach((e) => {
    e = r.data[e];
    (o.push({ name: e.x, data: e.quarters }), s.push(theme(e.colorToken)));
  }),
...
let getYearlyOptions = () => {
    var e = makeData(),
      t = e.map((e) => theme(e.colorToken)),
      a = (e) => {
        var t = document.querySelector(e);
        return t || (console.error(`Element with id ${e} not found`), null);
      };
    let r = a("#chart-quarter"),
      o = a("#chart-year");
    return {
      series: [{ data: e }],
      chart: {
        ...
        type: "bar",
        events: {
          dataPointSelection: function (e, t, a) {
            r &&
              o &&
...
if (yearChart && quarterChart) {
  let s = new ApexCharts(yearChart, getYearlyOptions()),
    e = (s.render(), new ApexCharts(quarterChart, getQuarterlyOptions())),
    r = (e.render(), document.querySelector("#chart-quarter")),
    o = document.querySelector("#chart-year"),
    t =
      ((r && o) ||
        console.error("Elements chart-quarter or chart-year not found"),
      s.addEventListener("dataPointSelection", function (e, t, a) {
        (1 !== a.selectedDataPoints[0].length ||
          r.classList.contains("active") ||
          (o.classList.add("chart-quarter-activated"),
          r.classList.add("active")),
      ...
      s.addEventListener("updated", function (e) {
        updateQuarterChart(e, "barQuarter");
      }),
      document.querySelector("#model")),
    a =
      (t || console.error('Element with id "model" not found'),
      t.addEventListener("change", function (e) {
        s.updateSeries([{ data: makeData() }]);
      }),
      new MutationObserver(() => {
        let e = s.w.globals.selectedDataPoints?.[0] || [],
          t = document.querySelector("#chart-year"),
          a = document.querySelector("#chart-quarter");
        ((t && a) ||
          console.error("Elements chart-year or chart-quarter not found"),
          s.updateOptions(getYearlyOptions()));
        var r = s.w.globals.intervalId,
          o = s.w.globals.timeoutId;
        (r && clearInterval(r),
          o && clearTimeout(o),
          (s.w.globals.intervalId = setTimeout(() => {
            (0 < e.length
              ? (e.forEach((e) => s.toggleDataPointSelection(e)),
                updateQuarterChart(s, "barQuarter"),
                t.classList.add("chart-quarter-activated"),
                a.classList.add("active"))
              : (t.classList.remove("chart-quarter-activated"),
                a.classList.remove("active")),
              clearInterval(s.w.globals.intervalId),
              delete s.w.globals.intervalId);
          }, 100)));
      }));
