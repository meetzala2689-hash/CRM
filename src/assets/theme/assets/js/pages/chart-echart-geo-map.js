let worldMap = new CustomEChart({
  ...
  usaMap =
    ...
    document.getElementById("usa-map")),
  mapBarMorphing =
    (usaMap &&
      fetch("assets/data/usa_geo.json")
        .then((e) => e.json())
        .then((e) => {
          echarts.registerMap("USA", e, {
            Alaska: { left: -131, top: 25, width: 15 },
            Hawaii: { left: -112, top: 25, width: 5 },
            "Puerto Rico": { left: -76, top: 26, width: 2 },
          });
          let t = new CustomEChart({
            selector: usaMap,
            options: () => ({
              tooltip: {
                trigger: "item",
          ...
          t.element.addEventListener("click", () => {
            t.chart.dispatchAction({ type: "restore" });
          });
        })
        .catch((e) => console.log(e)),
    document.getElementById("map-bar-morphing")),
  pieChartOnMap =
    (mapBarMorphing &&
      fetch("assets/data/usa_geo.json")
        .then((e) => e.json())
        .then((e) => {
              ...
              tooltip: {
                ...
                formatter: function (e) {
                  return null != e.value && "-" !== e.value
                    ? `<strong>${e.name}</strong><br/>Population: ` +
                        e.value.toLocaleString()
                    : `<strong>${e.name}</strong><br/>No data`;
                },
              },
            };
          }
          function o() {
            return {
              tooltip: {
                ...
                formatter: function (e) {
                  var t = e[0].name;
                  let a = `<div style="font-size: 14px; font-weight: 600; text-transform: uppercase; border-bottom: 1px solid ${theme("border-color")}; margin-bottom: 8px; padding: 3px 10px 8px;">${t}</div>`;
                  return (
                    e.forEach((e) => {
                      a += `<div style="margin-top: 4px; padding: 3px 15px;">
              ...
              xAxis: {
                ...
                splitLine: { show: !0, lineStyle: { color: theme("light") } },
              },
              yAxis: {
                type: "category",
                axisLabel: {
          ...
          setInterval(function () {
            ((r = "map" === r ? "bar" : "map"),
              n.chart.setOption(("map" === r ? a : o)(), !0));
          }, 3e3);
        })
        .catch((e) => console.log(e)),
    document.getElementById("pie-chart-on-map")),
  geoSVGScatterMap =
    (pieChartOnMap &&
      fetch("assets/data/usa_geo.json")
        .then((e) => e.json())
          ...
          (echarts.registerMap("USA", e, {
            ...
            new CustomEChart({
              ...
              options: () => ({
                ...
                series: [
                  t([-86.753504, 33.01077], 15),
                  t([-116.853504, 39.8], 25),
                  t([-99, 31.5], 30),
                  t(
                    540 < +echarts.version.split(".").slice(0, 3).join("")
                      ? "Maine"
                      : [-69, 45.5],
                    12,
                  ),
        ...
        .catch((e) => console.log(e)),
    document.getElementById("geo-svg-scatter-map"));
geoSVGScatterMap &&
  fetch("assets/images/svg/iceland.svg")
    .then((e) => e.text())
    .then((e) => {
      echarts.registerMap("iceland", { svg: e });
      let t = new CustomEChart({
      ...
      t.chart.on("selectchanged", function (e) {
        e.selected.length
          ? ((e = e.selected[0].dataIndex[0]),
            (e = option.series.data[e][2]),
            t.chart.dispatchAction({ type: "geoSelect", geoIndex: 0, name: e }),
            t.chart.dispatchAction({ type: "showTip", geoIndex: 0, name: e }))
          : (t.chart.dispatchAction({ type: "hideTip" }),
            t.chart.dispatchAction({ type: "geoSelect", geoIndex: 0 }));
      });
    })
    .catch((e) => console.log(e));
